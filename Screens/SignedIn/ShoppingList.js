...
useEffect (() => {
    async function loadPoints() {
        const Points = await AsyncStorage.getItem('count1');
...
useEffect(() => {
    ...
    const fetchTasks = async () => {
        try {
            const Items = await AsyncStorage.getItem('Items'); //Fetches the tasks stored within the async storage 
      if (Items !==null) {
...
const AddItem = async () => {
      try {
          if(Items.length <= 7){ //Stops the list being able to exeed 7 to fit with the page design
...
const  saveItem = (index) => {
        ...
        setItems(itemsCopy);
        AsyncStorage.setItem('Items',JSON.stringify(itemsCopy));
};
//runs when the user touches the Task (which is formatted through the Task.js file)
const Complete =  (index) => {
    ...
    let amountToAdd = 2; //this deals with the addition of points to the grading system
    if (Points >=10 && Points < 20) {
        amountToAdd =1.5;
    } else if (Points >=20 && Points < 30) { //Shopping is worth more points initially however this descreases over time to insentivise mixing up what you complete
        amountToAdd =1;
    }else if (Points >=25) {
        amountToAdd =0.5;
    }
setPoints (Points + amountToAdd);
AsyncStorage.setItem('count1', (Points + amountToAdd).toString());
}
...
return (
    ...
    <KeyboardAvoidingView
      ...
      <TextInput style={styles.input} placeholder={'Add to shopping list...'} value={task} onChangeText={text => setTask(text)}/>
